...
inputEnter.forEach(
    (item,index)=> item.addEventListener('click', () => handleSearch(index))
...
function handleSearch(index){
 ...
 function pullEspecificPokemon(index){
    ...
    pokemonTotalNumber.innerHTML = '0 pokemons'
        ...
        axios({
            url: 'https://pokeapi.co/api/v2/pokemon/'+ searchedPokemon[index].value.toLowerCase(),
            
        }).then(r => r.data).then(pokemon => {
            resetCardList()
